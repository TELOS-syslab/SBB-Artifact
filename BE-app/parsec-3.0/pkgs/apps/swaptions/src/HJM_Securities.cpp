...
// Global counter for completed swaptions
static std::atomic<int> completed_swaptions(0);
volatile sig_atomic_t sigint_received = 0;
// Array to track which swaptions have been completed
static std::atomic<bool> *swaption_completed = nullptr;
// Global counter for completed trials (a/b format)
std::atomic<long> completed_trials(0);
long total_trials = 0;

// Signal handler for Ctrl+C
static void sigint_handler(int sig) {
  sigint_received = 1;
  long completed = completed_trials.load();
...
void * worker(void *arg){
  ...
  if (sched_getaffinity(0, sizeof(process_cpuset), &process_cpuset) == 0) {
    // Find available CPUs from the process affinity mask
    int cpu_count = 0;
    int available_cpus[CPU_SETSIZE];
    for (int i = 0; i < CPU_SETSIZE; i++) {
      if (CPU_ISSET(i, &process_cpuset)) {
        available_cpus[cpu_count++] = i;
      }
    }
    // Assign this thread to a specific CPU (round-robin)
    if (cpu_count > 0) {
      int assigned_cpu = available_cpus[tid % cpu_count];
  ...
  for(int i=beg; i < end; i++) {
     ...
     swaptions[i].dSimSwaptionMeanPrice = pdSwaptionPrice[0];
     swaptions[i].dSimSwaptionStdError = pdSwaptionPrice[1];  
     swaption_completed[i].store(true);  // Mark this swaption as completed
     completed_swaptions.fetch_add(1);
#if SHENANGO
     pthread_yield();
#endif
   }
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
    ...
    // Try to get existing shared memory segment
    int shmid = shmget(key, sizeof(uint64_t) * nThreads * 8, 0777);
    if (shmid == -1) {
      // If it doesn't exist or size mismatch, try to remove old one and create new
      int old_shmid = shmget(key, 0, 0);
      if (old_shmid != -1) {
        shmctl(old_shmid, IPC_RMID, NULL);
      }
      // Create new shared memory segment
      shmid = shmget(key, sizeof(uint64_t) * nThreads * 8, 0777 | IPC_CREAT);
    }
    
    if (shmid == -1) {
      perror("shmget failed");
      fprintf(stderr, "Failed to create shared memory segment. Key: 0x%x, Size: %zu\n", 
              key, sizeof(uint64_t) * nThreads * 8);
      exit(1);
    }
    ...
    if (count == (void *)-1) {
      ...
      exit(1);
        ...
        // Initialize completion tracking array
        swaption_completed = new std::atomic<bool>[nSwaptions];
        for (i = 0; i < nSwaptions; i++) {
          swaption_completed[i].store(false);
        }
	...
	completed_swaptions.store(0);
	completed_trials.store(0);
	total_trials = (long)nSwaptions * (long)NUM_TRIALS;
	...
	__parsec_roi_end();
        ...
        // Normal output: all swaptions (if Ctrl+C was received, we already exited in signal handler)
        for (i = 0; i < nSwaptions; i++) {
	  ...
	  free_dmatrix(swaptions[i].ppdFactors, 0, swaptions[i].iFactors-1, 0, swaptions[i].iN-2);
        ...
        // Clean up completion tracking array
        if (swaption_completed) {
          delete[] swaption_completed;
          swaption_completed = nullptr;
        }
